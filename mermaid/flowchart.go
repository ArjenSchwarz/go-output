...
// Edge represents an edge connecting two nodes in a flowchart
type Edge struct {
	From *Node // Pointer to the Node where the Edge starts.
	To   *Node // Pointer to the Node where the Edge ends.
}

// NewFlowchart creates a new flowchart with the given settings
...
// AddEdgeByNames adds an edge between two nodes identified by their names
...
// RenderString renders the flowchart as a Mermaid string
