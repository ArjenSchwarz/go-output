...
- The byte-level tabular transformers no longer falsely advertise HTML support (T-1510). `SortTransformer.CanTransform`, `LineSplitTransformer.CanTransform`, and `FormatDetector.IsTabularFormat` — and everything derived from it: `SupportsSorting`, `SupportsLineSplitting`, the `FormatAwareTransformer` sort/linesplit gating, and `EnhancedSortTransformer` — now return `false` for `html`. These transformers only parse CSV (via `encoding/csv`) or line-based tab/comma/pipe table text; rendered HTML tables use markup instead of those separators, so sorting on HTML was always a silent no-op, and line splitting could corrupt raw HTML whenever cell text contained the configured separator (a comma inside cell text was mistaken for a column separator and the row was mangled across multiple lines). **Behavior change:** pipelines that register these transformers no longer run them on HTML output — the previously no-op sort pass disappears (no byte change) and the previously corrupting line-split pass now leaves HTML intact. Emoji transformation for HTML is unaffected. HTML-aware table transformation would be a new feature and remains out of scope.
