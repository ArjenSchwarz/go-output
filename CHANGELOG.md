...
- Built documents can no longer be mutated through their section contents (T-1543). `Builder.Build()` now freezes every section reachable from the document (including sections nested in other sections or in collapsible sections), so calling `SectionContent.AddContent` on a section obtained from a built document is a silent no-op instead of changing rendered output or racing with concurrent renders. `Clone()` a section to get a mutable copy; the building phase is unaffected.
