...
// TestStdoutWriterSetWriterNil is a regression test for T-1387.
...
func TestStdoutWriterSetWriterNil(t *testing.T) {
	...
	t.Run("zero-value writer returns error instead of panicking", func(t *testing.T) {
		...
		// A zero-value StdoutWriter never went through NewStdoutWriter, so
		// its internal writer is nil. Write must fail with a WriteError
		// rather than panic.
		sw := &StdoutWriter{baseWriter: baseWriter{name: "stdout"}}
